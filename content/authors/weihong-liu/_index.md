---
title: 刘伟鸿
role: 博士（已毕业）
name_en: Weihong Liu
superuser: false
organizations:
  - name: 中国科学技术大学 苏州高等研究院
    url: https://sz.ustc.edu.cn/
slug: weihong-liu
user_groups:
  - 已毕业校友
bio: 博士毕业于中国科学技术大学苏州高等研究院。研究方向为云边协同计算与边缘智能， 在 DAC、TCAD 等 CCF-A 会议期刊发表多篇论文（Sniper、Ace-Sniper 等），现就职于华为。
summary: 2025届，现就职于华为
interests:
  - 云边协同计算
  - 边缘智能
education:
  courses:
    - course: 博士, 中国科学技术大学苏州高等研究院 软件工程, 2021（导师：朱宗卫）
      institution: 中国科学技术大学苏州高等研究院
      year: 2021
social:
  - icon: envelope
    icon_pack: fas
    link: "#"
  - icon: github
    icon_pack: fab
    link: "#"
  - icon: graduation-cap
    icon_pack: fas
    link: "#"
  - icon: orcid
    icon_pack: fab
    link: "#"
  - icon: globe
    icon_pack: fas
    link: "#"
weight: 40
advisor: 朱宗卫
enrollment: 2021
papers:
  - title: "Sniper: cloud-edge collaborative inference scheduling with neural network similarity modeling"
    authors: Weihong Liu, Jiawei Geng, Zongwei Zhu*, Jing Cao, Zirui Lian
    venue: DAC 2022
    year: 2022
    level: CCF-A
    url: "https://doi.org/10.1145/3489517.3530474"
  - title: "Ace-Sniper: Cloud-Edge Collaborative Scheduling Framework with DNN Inference Latency Modeling on Heterogeneous Devices"
    authors: Weihong Liu, Jiawei Geng, Zongwei Zhu*, Yang Zhao, Cheng Ji, Changlong Li, Zirui Lian, Xuehai Zhou
    venue: IEEE TCAD
    year: 2024
    level: CCF-A
---

## 个人简介

博士毕业于中国科学技术大学苏州高等研究院。研究方向为云边协同计算与边缘智能，
在 DAC、TCAD 等 CCF-A 会议期刊发表多篇论文（Sniper、Ace-Sniper 等），现就职于华为。



## 研究方向

- 云边协同计算

- 边缘智能


## 教育背景

- 2021，博士，中国科学技术大学苏州高等研究院，软件工程，导师：朱宗卫


**导师**：朱宗卫

**入学年份**：2021

## 代表论文

- 1. **Weihong Liu**, Jiawei Geng, Zongwei Zhu*, Jing Cao, Zirui Lian. *Sniper: cloud-edge collaborative inference scheduling with neural network similarity modeling*. DAC 2022, 2022。CCF-A会议
  [链接](https://doi.org/10.1145/3489517.3530474)
- 2. **Weihong Liu**, Jiawei Geng, Zongwei Zhu*, Yang Zhao, Cheng Ji, Changlong Li, Zirui Lian, Xuehai Zhou. *Ace-Sniper: Cloud-Edge Collaborative Scheduling Framework with DNN Inference Latency Modeling on Heterogeneous Devices*. IEEE TCAD, 2024。CCF-A会议


**毕业去向**：华为

**2025届**
