...
头像支持 `avatar.jpg` 或 `avatar.png`，**图片名称必须为`avatar`**，建议使用清晰的近正方形照片，相较于常用的证件照可以上下裁剪一下，不然容易额头被挡住；没有头像时网站会显示姓名首字。
...
| `title` | 是 | 中文姓名 |
| `name_en` | 建议 | 英文姓名 |
| `slug` | 是 | 唯一标识，必须与成员目录名一致，**小写，使用连字符而不是空格** |
...
| `enrollment` | 学生必填 | 入学年份，用于显示“20xx级”和在读成员排序，**硕博连读的博士请填写博士入学年份** |
